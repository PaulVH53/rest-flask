...
app = Flask(__name__)
api = Api(app)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(os.getcwd(), 'todo_api/sqlite.db')
...
class ToDo(Resource):
    ...
    def get(self, todo_id):
        ...
        if not task:
            abort(404, description="Aborted. Could not find task with that id")
        return task
    ...
    def post(self, todo_id):
        ...
        task = ToDoModel.query.filter_by(id=todo_id).first()
        if task:
            abort(409, description="Aborted. Task ID already taken")
    ...
    def put(self, todo_id):
        ...
        task = ToDoModel.query.filter_by(id=todo_id).first()
        if not task:
            abort(404, description="Aborted. Task doen't exist, cannot update")
        # check if args task and/or summary has been passed to be updated
        if args['task']:
            task.task = args["task"]
        if args['summary']:
    ...
    def delete(self, todo_id):
        task = ToDoModel.query.get_or_404(todo_id)
